...
mod aws_ecs_service_scaling_test {
    ...
    const COMPONENT_ECS_SERVICE_SCALING_FILE_PATH: &str = "./tests/yaml/component_ecs_service_scaling.yaml";
    ...
    async fn aws_ecs_api_service_desired_count_update_test() -> Result<()> {
        ...
        let desired_count = 2;
        let parse_result = read_yaml_file(COMPONENT_ECS_SERVICE_SCALING_FILE_PATH).unwrap();
